...
def test_crear_categoria(client, auth_headers):
    ...
    create_resp = client.post('/api/v1/categoria/register', json={
        "nombre_categoria": "categoria example"   }, headers=auth_headers)
    assert create_resp.status_code in (200, 201)


def test_registrar_categoria_duplicada(client, sample_category ,auth_headers):
    create_resp = client.post('/api/v1/categoria/register', json={
        "nombre_categoria": "Electronics"}, headers=auth_headers)
    assert create_resp.status_code == HTTPStatus.CONFLICT


def test_registrar_categoria_sin_token(client, auth_headers):
    create_resp = client.post('/api/v1/categoria/register', json={ "nombre_categoria": "Electronics"})
    assert create_resp.status_code == HTTPStatus.UNAUTHORIZED


def test_consultar_categoria_por_id(client, auth_headers, categoria_id):
    create_resp = client.get(f'/api/v1/categoria/{categoria_id}', headers=auth_headers)
